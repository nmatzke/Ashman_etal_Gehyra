...
wd = "/drives/GDrive/__GDrive_projects/2016-07-31_divide_and_conquer_starBEAST/05_Gehyra_106_loci/v1_subgroup1_var+punct/"
setwd(wd)


# The name of the Excel settings file
xlsfn = "settings_v1_varpunct.xlsx"
...
data_XML_preCompleteness_fn = "data_XML_preCompleteness.Rdata"
if (runslow)
	{
	...
	#######################################################
	output_preCompleteness = convert_data_XML_to_dnalists(data_XML=data_XML_preCompleteness, remove_taxon=TRUE, string_to_delete="_seqfrom_", name_seqs_by_taxonname=TRUE, use_single_unique_seqName_for_alignment_name=TRUE)
	
	# Calculate completeness, and write stats and datasets, before completeness filtering
	completeness_df_preCompleteness = completeness_stats_from_data_XML(data_XML_preCompleteness)
	write.table(x=completeness_df_preCompleteness, file="completeness_df_preCompleteness.txt", append=FALSE, quote=FALSE, sep="\t", row.names=TRUE, col.names=TRUE)
	
	# Write out the data files BEFORE completeness filtering
	outfns_preCompleteness = write_DNAdf_to_files(dna_taxa_list=output_preCompleteness$dna_taxa_list, dna_dfs_list=output_preCompleteness$dna_dfs_list, datatypes=output_preCompleteness$datatypes, suffix="SavedOut", write_unmerged_also_TF=TRUE, outdir="z_saved_preCompleteness")
	
	data_XMLs_preCompleteness = data_XMLs
	save(data_XMLs_preCompleteness, file=data_XML_preCompleteness_fn)
	save(output_preCompleteness, file="dnalist_conversion_output_preCompleteness.Rdata")
	...
	write.table(x=completeness_df, file="completeness_df.txt", append=FALSE, quote=FALSE, sep="\t", row.names=TRUE, col.names=TRUE)

	# Read the data_XML to a dnalist, for filtering based on completeness etc.
	output = convert_data_XML_to_dnalists(data_XML=data_XMLs$data_XML, remove_taxon=TRUE, string_to_delete="_seqfrom_", name_seqs_by_taxonname=TRUE, use_single_unique_seqName_for_alignment_name=TRUE)

	# Write out the data to a bunch of files
	outfns = write_DNAdf_to_files(dna_taxa_list=output$dna_taxa_list, dna_dfs_list=output$dna_dfs_list, datatypes=output$datatypes, suffix="SavedOut", write_unmerged_also_TF=TRUE, outdir="z_saved")
	
	save(data_XMLs, file=data_XML_fn)
	save(output, file="dnalist_conversion_output.Rdata")
	} else {
	# Loads to "output_preCompleteness"
	load("dnalist_conversion_output_preCompleteness.Rdata")

	# Loads to "output"
	load("dnalist_conversion_output.Rdata")
	
	# Loads to "data_XMLs_preCompleteness"
	load(file=data_XML_preCompleteness_fn)
...
unname(output$dna_alignment_lengths)
...
# Completeness statistics after filtering
data_XMLs$completeness_df
...
write.table(x=DNA_columns_total_number, file="DNA_columns_total_number.txt", append=FALSE, quote=FALSE, sep="\t", row.names=TRUE, col.names=TRUE)
...
if (StarBeast2_TF == FALSE)
	...
	xml = make_BDSKY_model(treemodel_df, tree_name=tree_name, clockModel_name=clockModel_name, alignment_name_w_taxa=alignment_name_w_taxa, tipDates_id="tipDates", xml=xml, XML_mod_for_cont_chars=XML_mod_for_cont_chars, xlsfn=xlsfn)
...
dataset_source = ' Dataset: filtered Gehyra loci; BEASTmasteR by Nick Matzke '
